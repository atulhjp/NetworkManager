 ...
 * are pushed into PacRunner. If "manual" is selected then users can specify diffe-
 * rent proxy servers for different protocols. If "none" is selected then no proxy
 * configuration is given to PacRunner to fulfill client queries.
 **/

G_DEFINE_TYPE_WITH_CODE (NMSettingProxy, nm_setting_proxy, NM_TYPE_SETTING,
                         _nm_register_setting (PROXY, 1))
