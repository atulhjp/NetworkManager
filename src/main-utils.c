...
nm_main_utils_ensure_rundir ()
{
	...
	nm_assert (g_str_has_prefix (NM_DEVICE_STATE_DIR, NMRUNDIR"/"));
	if (g_mkdir (NM_DEVICE_STATE_DIR, 0755) != 0) {
		errsv = errno;
		fprintf (stderr, _("Cannot create '%s': %s"), NM_DEVICE_STATE_DIR, g_strerror (errsv));
		exit (1);
	}
}
