                  ...
                  guint *out_call_id)
{
	...
	if (blocking) {
		...
		ret = _nm_dbus_proxy_call_sync (dispatcher_proxy, "Action",
		                                g_variant_new ("(s@a{sa{sv}}a{sv}a{sv}a{sv}a{sv}a{sv}@a{sv}@a{sv}sa{sv}a{sv}a{sv}b)",
		                                               action_to_string (action),
		                                               connection_dict,
		                                               &connection_props,
		                                               &device_props,
                        ...
                        guint *out_call_id)
{
	return _dispatcher_call (action, FALSE, settings_connection, applied_connection, parent_device, vpn_iface,
	                         vpn_proxy_config, vpn_ip4_config, vpn_ip6_config, callback, user_data, out_call_id);
}
